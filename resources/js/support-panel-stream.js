        ...
        try {
            ...
            if (! response.ok) {
                window.Livewire?.dispatch('padmission-support-stream-error', {
                    message: 'Support AI could not start. Please try again or open a ticket.',
                })
            ...
            await parseSse(response, {
                ...
                complete: () => window.Livewire?.dispatch('padmission-support-stream-complete'),
                error: (data) => {
                    window.Livewire?.dispatch('padmission-support-stream-error', {
                        message: data.message || 'Support AI could not finish the response.',
                    })
                    window.Livewire?.dispatch('padmission-support-stream-complete')
                },
                done: () => window.Livewire?.dispatch('padmission-support-stream-complete'),
            })
        } catch (error) {
            window.Livewire?.dispatch('padmission-support-stream-error', {
                message: 'Support AI could not connect. Please try again or open a ticket.',
            })
            window.Livewire?.dispatch('padmission-support-stream-complete')
            return
        }
