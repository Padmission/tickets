...
class CopilotPlugin implements Plugin
{
    ...
    /**
     * Register the resolver used to scope Copilot records to the consuming app's
     * current tenant. The resolver should return ['type' => <tenant model class>,
     * 'id' => <tenant key>] or null when no tenant is current.
     */
    public function copilotTenantResolver(?Closure $resolver): static
    {
        CopilotTenantContext::resolveUsing($resolver);
