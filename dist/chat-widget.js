(()=>{var p=Object.defineProperty;var w=(t,s,e)=>s in t?p(t,s,{enumerable:!0,configurable:!0,writable:!0,value:e}):t[s]=e;var u=(t,s,e)=>(w(t,typeof s!="symbol"?s+"":s,e),e);var d=class extends HTMLElement{constructor(){super();u(this,"_useShadowDom",null);this.useShadowDom&&this.attachShadow({mode:"open"}),this._listeners=[]}get useShadowDom(){return this._useShadowDom!==null?this._useShadowDom:this.hasAttribute("use-shadow-dom")?(this._useShadowDom=this.getAttribute("use-shadow-dom")!=="false",this.removeAttribute("use-shadow-dom"),this._useShadowDom):!0}async connectedCallback(){this._initializeAttributes(),this._initializeStylesheet(),await this._render()}disconnectedCallback(){for(let{elem:e,event:i,callback:o}of this._listeners)e?.removeEventListener(i,o)}dispatch(e,i={}){let o=new CustomEvent(e,{bubbles:!0,composed:!0,detail:i});window.dispatchEvent(o)}changeView(e,i={}){this.dispatch("change-view",{viewName:e,attributes:i})}rootNode(){return this.useShadowDom?this.shadowRoot:this}_initializeStylesheet(){if(!this.stylesheet)return;let e=document.createElement("link");e.rel="stylesheet",e.href=this.stylesheet,this.rootNode().appendChild(e)}_initializeAttributes(){this.getAttributeNames().forEach(i=>{let o=i.replace(/-([a-z])/g,n=>n[1].toUpperCase()),r=this.getAttribute(i);r!==null&&(this[o]=r)})}_configureEventListeners(e){let i,o=[],r=document.createNodeIterator(e,NodeFilter.SHOW_ELEMENT,{acceptNode:n=>{if(!(n instanceof HTMLElement))return NodeFilter.FILTER_REJECT;if(n.tagName.includes("-")&&n.tagName!==this.tagName)return o.push(n),NodeFilter.FILTER_REJECT;for(let l of o)if(l.contains(n))return NodeFilter.FILTER_REJECT;return NodeFilter.FILTER_ACCEPT}});for(;i=r.nextNode();){if(!i)return;for(let n of i.attributes)n.name.startsWith("@")&&this._processEventHandler(n)}for(let{elem:n,event:l,callback:m}of this._listeners)n?.addEventListener(l,m)}_processEventHandler(e){let i=e.ownerElement,{name:o,value:r}=e;this._listeners.push({elem:i,event:o.slice(1),callback:n=>this[r](n)}),i.removeAttributeNode(e)}async _render(){if(!this.render)throw new Error("Web components extending BaseElement must implement a `render` method.");this.beforeRender&&this.beforeRender();let e=await this.render();this.afterRender&&this.afterRender(e),this._configureEventListeners(e),this.bindListeners&&this.bindListeners(e),this.useShadowDom?this.shadowRoot.appendChild(e):this.appendChild(e),this.renderedCallback&&this.renderedCallback()}},c=d;function a(t){let s=document.createElement("template");return s.innerHTML=t,s.content.cloneNode(!0)}customElements.define("chat-view-ticket",class extends c{get useShadowDom(){return!1}renderedCallback(){window.addEventListener("ticket-created",t=>{this.querySelector("[data-chat-subject]").innerHTML=t.detail.subject})}back(){this.changeView("chat-list-tickets")}render(){return a(`
            ...
            `)}});async function h(t,s={},e="GET"){e==="GET"&&(t+="?"+new URLSearchParams(s).toString());let i=await fetch(t,{method:e,headers:{"Content-Type":"application/json",Accept:"application/json","X-Requested-With":"XMLHttpRequest","X-CSRF-TOKEN":document.querySelector('meta[name="csrf-token"]')?.getAttribute("content")||""},credentials:"same-origin",body:e==="POST"?JSON.stringify(s):null});if(!i.ok){let o=await i.text();throw console.error("HTTP error response",{status:i.status,text:o}),new Error("HTTP Error")}return await i.json()}customElements.define("chat-list-tickets",class extends c{get useShadowDom(){return!1}async renderedCallback(){let t=await this.fetchTickets(),s=a(`
